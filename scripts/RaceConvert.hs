...
processRows row acc =
  case partition (\ (a, b) -> a == "raceID") row of
    ([("raceID", Int fid)], rest) ->
      HM.insert (text fid) (Object (convertFields rest)) acc
    _ ->
      error ("Couldn't find raceID?")
...
run =
  do sql <- openReadonlyConnection "/Users/awick/Downloads/sqlite-latest.sqlite"
     Right rows <- execStatement sql "SELECT * FROM chrRaces"
     let res = foldr processRows HM.empty (concat rows)
     encodeFile "static/raceInfo.yaml" (Object res)
